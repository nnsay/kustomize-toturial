...
const pod = async () => {
  ...
  const podsRes = await k8sApi.listNamespacedPod("tutorial");
  console.log(podsRes.body);

  const result = await k8sApi.createNamespacedPod("tutorial", {
    apiVersion: "v1",
    kind: "Pod",
    metadata: {
      name: `${Date.now()}-ubuntu`,
    },
    spec: {
      containers: [
        {
...
const job = async () => {
  ...
  const { body } = await batchV1Api.createNamespacedJob("tutorial", job);
  console.log(body);
};

job().catch((err) => {
  console.error("k8s api error:");
